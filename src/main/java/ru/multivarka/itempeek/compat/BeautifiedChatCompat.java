...
public final class BeautifiedChatCompat {
    ...
    private boolean init(){if(initialized)return available;initialized=true;if(!ModList.get().isLoaded("beautifiedchatserver"))return false;try{Class<?> c=Class.forName("com.natamus.beautifiedchatserver_common_neoforge.config.ConfigHandler");timestampFormat=c.getField("timestampFormat");chatMessageFormat=c.getField("chatMessageFormat");showRankTitles=c.getField("showRankTitles");rankTitleFormat=c.getField("rankTitleFormat");Class<?> u=Class.forName("com.natamus.beautifiedchatserver_common_neoforge.util.Util");colour=u.getMethod("getColour",String.class,String.class);rank=u.getMethod("getRankOfPlayer",String.class);available=true;return true;}catch(Throwable e){return false;}}
}
