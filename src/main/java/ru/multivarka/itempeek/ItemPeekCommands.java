...
public final class ItemPeekCommands {
    ...
    public static void register(RegisterCommandsEvent event) {
        var root=Commands.literal("itempeek").requires(s->{if(ItemPeekConfigService.path()==null)ItemPeekConfigService.initialize(s.getServer());return s.permissions() instanceof net.minecraft.server.permissions.LevelBasedPermissionSet permissions && permissions.level().isEqualOrHigherThan(net.minecraft.server.permissions.PermissionLevel.byId(ItemPeekConfigService.snapshot().bypassPermissionLevel()));});
